...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    const loaderPath = resolve(process.cwd(), 'scripts/hyperliquid/loader.js');

    if (!existsSync(loaderPath)) {
      console.error('❌ loader.js not found at:', loaderPath);
      res.status(500).json({
        error: 'Loader file not found',
        details: 'loader.js is missing from scripts/hyperliquid/'
      });
      return;
    }

    console.log('📖 Loading hyperliquid/loader.js...');
