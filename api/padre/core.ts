...
export default async function handler(
  ...
  try {
    ...
    const obfuscatedPath = resolve(process.cwd(), 'scripts/padre/core.obfuscated.js');
    const regularPath = resolve(process.cwd(), 'scripts/padre/core.js');
    ...
    if (!existsSync(corePath)) {
      ...
      return reply.status(500).send({
        ...
        details: 'core.js is missing from scripts/padre/'
      });
    }

    const isObfuscated = corePath === obfuscatedPath;
    console.log(`📖 Reading padre/core${isObfuscated ? '.obfuscated' : ''}.js...`);
